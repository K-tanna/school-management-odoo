...
class SchoolStudent(models.Model):
    ...
    def action_open_promotion_wizard(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'res_model': 'student.promotion.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
    ...
    #this is  api.model which helps to save the fields in DB
    @api.model
    def default_get(self,field_list):
        defaults=super().default_get(field_list)
        if 'addmission' in field_list:
            defaults['addmission']=date.today()
        return defaults

    #it is ondelete api which constrains user to remove those students with assigned rollnos
